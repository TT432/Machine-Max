...
import io.github.tt432.machinemax.utils.physics.ode.DTriMesh;
import io.github.tt432.machinemax.utils.physics.ode.internal.DxGeom;
import io.github.tt432.machinemax.utils.physics.ode.internal.DxSpace;
import io.github.tt432.machinemax.utils.physics.ode.internal.gimpact.GimContact;
import io.github.tt432.machinemax.utils.physics.ode.internal.gimpact.GimDynArray;
...
abstract class DxMeshBase extends DxGeom implements DTriMesh {
    ...
    public DxMeshBase(DxSpace Space, DxTriDataBase Data, DTriCallback Callback,
                      DTriArrayCallback ArrayCallback, DTriRayCallback RayCallback, boolean doTCs) {
        super(Space, true);
        m_Callback = Callback;
    ...
    //public:
    public void assignCallback(DTriCallback value) {
        m_Callback = value;
    }

    public DTriCallback retrieveCallback() {
        return m_Callback;
    }

    @Deprecated
    public void assignArrayCallback(DTriArrayCallback value) {
        m_ArrayCallback = value;
    }

    @Deprecated
    public DTriArrayCallback retrieveArrayCallback() {
        return m_ArrayCallback;
    }

    public void assignRayCallback(DTriRayCallback value) {
        m_RayCallback = value;
    }

    public DTriRayCallback retrieveRayCallback() {
        return m_RayCallback;
    }

    public void assignTriMergeCallback(DTriTriMergeCallback value) {
        m_TriMergeCallback = value;
    }

    public DTriTriMergeCallback retrieveTriMergeCallback() {
    ...
    //	void dGeomTriMeshSetCallback(dGeomID g, dTriCallback* Callback)
    @Override
    public void setCallback(DTriCallback Callback) {
        dGeomTriMeshSetCallback(Callback);
    }

    //dTriCallback* dGeomTriMeshGetCallback(dGeomID g)
    @Override
    public DTriCallback getCallback() {
        return dGeomTriMeshGetCallback();
    }

    //void dGeomTriMeshSetArrayCallback(dGeomID g, dTriArrayCallback* ArrayCallback)
    @Deprecated
    public void setArrayCallback(DTriArrayCallback ArrayCallback) {
        dGeomTriMeshSetArrayCallback(ArrayCallback);
    }

    //dTriArrayCallback* dGeomTriMeshGetArrayCallback(dGeomID g)
    @Deprecated
    public DTriArrayCallback getArrayCallback() {
        return dGeomTriMeshGetArrayCallback();
    }

    //void dGeomTriMeshSetRayCallback(dGeomID g, dTriRayCallback* Callback)
    @Override
    public void setRayCallback(DTriRayCallback Callback) {
        dGeomTriMeshSetRayCallback(Callback);
    }

    //dTriRayCallback* dGeomTriMeshGetRayCallback(dGeomID g)
    @Override
    public DTriRayCallback getRayCallback() {
