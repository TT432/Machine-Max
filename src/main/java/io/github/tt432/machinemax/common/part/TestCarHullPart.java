...
//    public static final ResourceLocation PART_MODEL = ResourceLocation.fromNamespaceAndPath(MachineMax.MOD_ID, "entity/kv2/kv2_turret");
    ...
    public DVector3 getAerodynamicForceCoef(AbstractPart part) {
        //气动力相关系数
        double BASIC_AIRDRAG_COEF_ZP=2;//空气阻力系数(前向)，一般较小
        double BASIC_AIRDRAG_COEF_ZN=3;//空气阻力系数(后向)，一般较小
        double BASIC_AIRDRAG_COEF_XP=5;//空气阻力系数(左向)
        double BASIC_AIRDRAG_COEF_XN=5;//空气阻力系数(右向)
        double BASIC_AIRDRAG_COEF_YP=5;//空气阻力系数(上向)
        double BASIC_AIRDRAG_COEF_YN=0;//空气阻力系数(下向)
        double BASIC_AIRLIFT_COEF_Z =0.2;//空气升力系数(前向)，形状带来的额外升力
