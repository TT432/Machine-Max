...
public abstract class AbstractMMPart implements Iterable<AbstractMMPart>, MMPartPhysParametersC {
    //基础属性
    @Getter
    @Setter
    ...
    public Iterator<AbstractMMPart> iterator() {
        return new DFIterator();
    }

    class DFIterator implements Iterator<AbstractMMPart>{
        int index = 0;
        @Override
        public boolean hasNext() {
            ...
            if(index0 < PART_SLOT_NUM){//按顺序读取槽位，检查槽位状态
                if(AbstractMMPart.this.children_parts.get(index0).hasPart()){
                    return true;
        ...
        @Override
        public AbstractMMPart next() {
            index++;
            return AbstractMMPart.this.children_parts.get(index-1).getChildPart();
        }
    }
